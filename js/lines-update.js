...
function containsNumberToken(text) {
  ...
  return /NTN\s*[:#-]?\s*[A-Z0-9-]+/.test(value) || /CNIC\s*[:#-]?\s*[0-9-]+/.test(value) || /\b[0-9]{5}-[0-9]{7}-[0-9]\b/.test(value);
}
...
function cleanName(text) {
  ...
  value = value.replace(/NTN\s*[:#-]?\s*[A-Z0-9-]+(\([^)]*\))?/ig, " ");
  value = value.replace(/CNIC\s*[:#-]?\s*[0-9-]+/ig, " ");
...
function extractNumber(text) {
  ...
  let match = value.match(/NTN\s*[:#-]?\s*([A-Z0-9-]+)/i);
  if (match && match[1]) return match[1].trim();

  match = value.match(/CNIC\s*[:#-]?\s*([0-9-]+)/i);
  if (match && match[1]) return match[1].trim();
