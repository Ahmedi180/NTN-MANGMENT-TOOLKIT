...
function processRow(row) {
  ...
  const numberSource = findNumberSource(sameRowEntries);
...
// FINAL NAME RULE:
// Name only from Shipper Company.
// If Shipper Company is blank or number-only, then use Shipper Name.
// Never use name from Shpr Addl Addr
// Never use name from Shipper Address line 1
...
function containsNumberToken(text) {
  ...
  return /NTN[\s.:#-]*[A-Z0-9-]+/.test(value) ||
         ...
         /NTN\.[A-Z0-9-]+/.test(value); // Handling NTN.NO type patterns
}
...
function extractNumber(text) {
  ...
  match = value.match(/NTN\.[A-Z0-9-]+/i); // For NTN.NO or similar patterns
  if (match && match[0]) return match[0].replace('NTN.', '').trim();  // Extract number after 'NTN.'
