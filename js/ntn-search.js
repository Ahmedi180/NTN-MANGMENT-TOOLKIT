...
async function loadCompanies() {
  try {
    ...
    state.rows = Object.entries(data).map(([id, value]) => ({
      id,
      ref: value.ref || '',
    ...
    if (!snapshot.exists()) {
      showNotice('Database empty hai. Add Company ya Upload Excel se data add karo.');
    } else {
      showNotice('');
    }
  } catch (error) {
    console.error(error);
    showNotice('Database load nahi hua. Realtime Database rules test mode me rakho.');
  }
}
...
async function handleExcelUpload(event) {
  ...
  reader.onload = async function(e) {
    ...
    event.target.value = '';
    await loadCompanies();
    alert(`${count} record(s) imported successfully.`);
  };
